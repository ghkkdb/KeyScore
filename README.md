...
<div style="display: flex; gap: 20px;">
  <div style="flex: 1; text-align: center;">
    <h3>Windows 11 Fluent 主题</h3>
    <img src="B.png" alt="KeyScore Windows 11 Fluent 主界面" style="width: 100%;" />
  </div>
  <div style="flex: 1; text-align: center;">
    <h3>游戏电竞主题</h3>
    <img src="W.png" alt="KeyScore 游戏电竞主题主界面" style="width: 100%;" />
  </div>
</div>
...
- 钢琴卷帘支持量化落点编辑；选择模式可拖动框选音符，绘制模式可在空白网格按住并横向拖动，以当前网格精度直接绘制所需拍数的音符；按住 `Alt` 可临时切换另一种操作。尾格被音符占满时自动追加三个当前精度的网格并向后滚动。
...
- 播放前会比较 KeyScore 与目标程序的 Windows 完整性级别；目标权限更高时可经用户确认，以管理员身份重启并自动恢复当前曲谱和按键方案。
- 支持录制游戏中的实际键鼠演奏：`F8` 开始/完成，自动按当前 Profile 反向还原音符、和弦、休止、小节和持续按住模式下的连音。
- 可在“设置 → 快捷键”自定义录制、播放、停止和编辑器拍数正向/反向切换快捷键（默认 `D` / `A`）；新音符默认使用 `1/4` 拍，常用拍数、顺序与默认值也可自行设置。
...
打包流程README.md  Windows 测试版打包、验证与分发流程
```
...
完整的环境准备、产物检查和压缩分发步骤见 [打包流程README.md](打包流程README.md)。

## 游戏自动按键风险与免责声明

KeyScore 仅通过 Windows 标准 `SendInput` 接口模拟键盘和鼠标输入，不读取或修改游戏内存，不注入游戏进程，也不提供绕过反作弊、权限控制或平台检测的功能。但“未修改游戏进程”不代表一定符合目标游戏、服务器或平台的服务条款；部分产品可能把任何自动输入、宏、脚本或无人值守操作认定为违规行为。

使用前请注意：

- 不同游戏、服务器和平台对自动按键的规定不同，并可能随时调整；
- 自动输入可能被反作弊系统、行为检测或运营方识别，导致警告、功能限制、临时封禁、永久封禁或其他账号损失；
- 软件无法判断某个游戏是否允许使用，也不保证任何 Profile、曲谱、播放方式或使用频率不会触发检测；
- 请优先在离线模式、单人内容、私人环境或已明确允许自动演奏的场景中使用，不要用于获取不公平的竞技、经济或排名优势；
- 如果游戏规则禁止宏、脚本或自动化操作，请立即停止使用，并不要尝试规避检测；
- 用户应自行确认目标游戏及所在地区的规则和法律要求，并自行承担因安装、配置、运行或分发本软件产生的账号、数据、设备及其他风险。

在适用法律允许的范围内，项目作者和贡献者不对因使用或无法使用 KeyScore 导致的账号处罚、虚拟物品损失、数据损坏、服务中断或其他直接、间接损失承担责任。继续使用即表示用户已理解上述风险；如果不同意，请不要在游戏中启用自动按键功能。
