...
- 搜索、导入、编辑和删除本地文本曲谱，保存后按 `@title` 自动同步文件名；支持从 MIDI 自动选择音符最多的非鼓轨、提取同起点最高音并转换为 KeyScore 曲谱。
...
- 可在“设置 → 快捷键”自定义录制、播放和停止三个全局快捷键；支持单键及 `Ctrl`、`Alt`、`Shift`、`Win` 组合键，保存后立即生效。
...
- `direct_note`：按倍低/低/中/高/倍高的自然音及半音排成十行，每行横向排列 `1～7`，每个音符独立绑定。
